...
- [x] Task 1: Modal horizontal margin on mobile (add `px-4` to outer wrapper — `Modal` + `FeedbackModal`)
- [x] Task 2: Form action buttons — stacked full-width on mobile (`flex-col sm:flex-row sm:justify-end`)
- [x] Task 3: `IconButton` touch target — add `min-h-[44px] min-w-[44px]`
- [x] Task 4: Sidebar mobile nav touch target — `py-sm` → `py-md` on mobile links
- [x] Task 5: `TransactionFilters` clear button — `w-full sm:w-auto`
- [x] Task 6: Header `max-w-[1200px]` → `max-w-300` (token alignment)
- [x] Task 7: Manual test at 375px / 768px / 1280px + overflow check
